javascript:(function(){
    ...
    btn.onclick=()=>{drone=!drone;if(drone&&geofs.aircraft.instance){targetAlt=geofs.aircraft.instance.llaLocation[2];btn.innerText="Drone ON";}else btn.innerText="Drone OFF";};
    ...
    knob.addEventListener("pointerdown",e=>{e.target.setPointerCapture(e.pointerId);});
    knob.addEventListener("pointermove",e=>{
        if(!drone) return;
        const rect=stick.getBoundingClientRect();
        stickX=(e.clientX-rect.left-rect.width/2)/50;
        stickY=(rect.height/2-(e.clientY-rect.top))/50;
    ...
    knob.addEventListener("pointerup",e=>{stickX=0;stickY=0;knob.style.left="25px";knob.style.top="25px";});

    const slider=document.createElement("input");
    slider.type="range";slider.min="-1";slider.max="1";slider.step="0.01";slider.value="0";
    slider.style.position="fixed";slider.style.right="20px";slider.style.bottom="20px";
    slider.style.width="50px";slider.style.height="150px";slider.style.transform="rotate(-90deg)";
    slider.style.zIndex=9999;document.body.appendChild(slider);
